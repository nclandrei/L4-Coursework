...
\newcommand{\gethin}[1]{\marginpar{\footnotesize \color{red} {\bf Gethin:} \textsf{#1}}}
...
\gethin{figure \ref{fig:java-uni} is not referenced in the text}
\pagebreak
...
These principles ensured a good workflow with results that could be remarked immediately. \gethin{not sure what you mean by ``remarked''} We believe that a major part of the
...
needs to take into account a range of different perspectives. Therefore, architectural design and focus on simplicity is vital to the success of every software. \gethin{I do not get therefore at the start of this sentence} We have used UML
diagrams in order to visualize the system as well as possible before proceeding with the implementation. \gethin{add reference to UML diagrams and say something about why they were chosen?}
...
our HTML page the animation. \gethin{previous sentence needs rewording as does not make sense} This will be composed out of 2 components: the node manipulator which will colour, move,
...
700 commits~\cite{velocityjs}, what it primarily offers is transforms, loops, easings and colour animation. This sounds
...
rather impossible. \gethin{the circular dependency is not clear from what you have said}

Also, an issue with using any of the 3 options was that they are
manipulating elements which are part of the DOM (Document Object Model), while in our project the elements are part of
the canvas. Therefore only the canvas could be animated as a whole without being able to manipulate the individual elements inside.
...
the correct times and that they do not overlap.  \gethin{are you sure you mean line 27?}
...
The full questionnaire can be found in the Appendix. \gethin{make sure you have added the form to the appendix}
