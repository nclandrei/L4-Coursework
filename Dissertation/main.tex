...
Because we would work with algorithms on trees and graphs, drawing nodes and edges was a major part in our
...
It can be noticed that this function will set up a node and then invoking ``callback(nodeData)'' will immediately add it to the
graph/tree. However, there are event listeners that need to take input from the user such as the edge's weight. Listing~\ref{custom-event-listener} shows an example of this implementation.

\begin{lstlisting}[language={Java}, label={custom-event-listener},caption={Custom edge insertion event listener for Dijkstra's Shortest Path algorithm.}]
...
When invoking ``editEdgeCustom'', the user is presented with a modal inside Palgo where he/she can add the weight and
either save the changes or cancel the process. Figure~\ref{fig:edge-weight-modal} shows this functionality in action.

\begin{figure}[!ht]
\centering
\includegraphics[scale=0.3]{edge-weight-modal}
\caption{Edge insertion modal for Dijkstra's Shortest Path algorithm.}
\label{fig:edge-weight-modal}
\end{figure}
...
executed (in the case presented in Listing~\ref{prim-jarnik-redraw}, adding the node to the minimum spanning tree). As
...
Moreover, another implemented feature that was very popular was the landing page in the application - as
